...
class GridSearch:
    ...
    def other_eval(df: DataFrame = None, qIds: ndarray = None, nDCG_at: list = None) -> dict:
        ...
        for _, v in df.groupby("qId"):
            v = v.sort_values("w_score", ascending=False)
            _, y = v.iloc[:, 5:], asarray([v["w_score"].to_numpy()])

            random_lambdas = []
            for i in range(10):
                random_lambdas.append(asarray([v.sample(frac=1)["w_score"].to_numpy()]))

            perfect_lambdas = asarray([v.sort_values("w_score", ascending=False)["w_score"].to_numpy()])
            worst_lambdas = asarray([v.sort_values("w_score")["w_score"].to_numpy()])
